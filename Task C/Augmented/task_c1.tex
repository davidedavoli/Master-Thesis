...
\begin{defn}[Encoding of $\Nat$ over $\SS$]
\begin{align*}
...
\begin{remark}[Appropriateness of $\SS_\Nat$]
...
\item For every $n\in\Nat$ and for every $1\leq i\leq n$,
the function ${P^-}^n_i$ such that ${P^-}^n_i(x_1,\dots, x_n,\subo) = x_i$ \ $(1\leq i\leq
n)$
\item For every $n\in\Nat$, the function ${H^-}_n$ such that ${H^-}_n(x_1,\dots, x_n,\subo) = \subo$.
...
Given two polynomials $p: \Nat^m \to \Nat$ and $q: \Nat^m \to \Nat$ we say that $q$ is \emph{universally greater} than $p$ if and only if $\forall n_1, \ldots, n_m \in \Nat.p(n_1, \ldots, n_m) \le q(n_1, \ldots, n_m)$. In this case we write $p \lesssim q$.
\end{defn}
...
Given two polynomials $p_1: \Nat^m \to \Nat$ and $p_2: \Nat^m \to \Nat$ there exists a polynomial $q$ so that $p_1 \le q\land p_2 \le q$
\end{lemma}
