...
\begin{comment}
\begin{proof}
Suppose that the function $f$ above accesses its oracle. It can only happen throughout the function $f_q$. The first argument of the occurrences of $f_q$ in the body of $f$ is itself a $\POR$ function. This means that its size is bounded by a polynomial computed on the sizes of all the variables of $\sone_1, \ldots, \sone_n$, by lemma \ref{lemma:sizeofpor}. Take all these polynomials $q_0, \ldots q_k$ and sum them together obtaining $p$. Such function is itself a polynomial and it is greater or equal to all the $q_j$s. For this reason, it is the polynomial we were looking for: the indexes to which $\oone$ is accessed are bound by $p$. So if two oracles $\oone$ and $\oone'$ share the $p(\sone_1, \ldots, \sone_n)$-th prefix all the accesses to the function $f_q$ produce the same outcome. This means that the function called on the first oracle and the one called on the second oracle behave the same.
\end{proof}
\end{comment}
\begin{proof}
\begin{itemize}
...
\begin{defn}[semantics of $\SIMP$'s expressions]
The semantics of an expression $E \in \lang{\xp}$ is the smallest function $\sred: \lang{\xp} \times (\id \longrightarrow \{\zero, \one\}^*)\longrightarrow \{\zero, \one\}^*$ closed under the following rules:
\begin{center}
\vspace{12pt}
...
\vspace{12pt}
\AxiomC{$\langle e, \store, \rangle \sred \sigma$}
\AxiomC{$\langle f, \store, \rangle \sred \tau$}
\AxiomC{$\sigma \subseteq \tau$}
\TrinaryInfC{$\langle e \sqsubseteq f, \store\rangle \sred \one$}
\DisplayProof
\hspace{18pt}
\AxiomC{$\langle e, \store, \rangle \sred \sigma$}
\AxiomC{$\langle f, \store, \rangle \sred \tau$}
\AxiomC{$\sigma \not\subseteq \tau$}
\TrinaryInfC{$\langle e \sqsubseteq f, \store\rangle \sred \zero$}
\DisplayProof
...
\AxiomC{$\langle f, \store, \rangle \sred \one$}
\BinaryInfC{$\langle e \land f, \store\rangle \sred \one$}
\DisplayProof
\hspace{18pt}
\AxiomC{$\langle e, \store, \rangle \sred \sigma$}
\AxiomC{$\langle f, \store, \rangle \sred \tau$}
\AxiomC{$\sigma \neq \one \land \tau \neq \one$}
\TrinaryInfC{$\langle e \land f, \store\rangle \sred \zero$}
\DisplayProof
\hspace{18pt}
...
\begin{defn}[Operational semantics of $\SIMP$]
